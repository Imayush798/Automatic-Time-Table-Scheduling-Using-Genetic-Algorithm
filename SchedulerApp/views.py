...
def timetable(request):
    ...
    while (schedule.getFitness() != 1.0) and (VARS['generationNum'] < 1000):
        if VARS['terminateGens']:
            # Calculate accuracy after termination
            accuracy = calculate_accuracy(schedule)
            print(f"Accuracy of Timetable: {accuracy}")
            return HttpResponse('')
...
def calculate_accuracy(schedule):
    # Calculate accuracy based on your defined formula
    # For example, you can calculate the percentage of conflicts resolved
    total_conflicts = schedule.getNumbOfConflicts()
    total_classes = len(schedule.getClasses())
    accuracy = ((total_classes - total_conflicts) / total_classes) * 100
    return accuracy
